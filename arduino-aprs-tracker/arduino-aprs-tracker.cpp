...
// Descriptions taken from http://www.aprs.net.au/vhf/what-smart-beaconing/
// LOW_SPEED 5 [km/h] //This is the speed below which you're effectively still
// HIGH_SPEED 90 //This is the speed above which you will transmit at the "Fast Rate".
// SLOW_RATE 1750 [sec] //This is how often your unit transmits when it's still (or nearly still - any speed below "Slow Speed")
// FAST_BEACON_RATE 175 //This is how often your unit transmits when it's moving at any speed greater than "Fast Speed".
// TURN_MIN 30 [deg] //This is the angle by which you must change heading by in order to transmit a beacon.
// TURN_SLOPE 240 //This is a calculation of turn angle versus speed  (angle/speed)
// MIN_TURN_TIME 20 //This is how often your unit transmits when it's continuously changing direction.
...
#define MIN_TURN_TIME 15

#else

#define LOW_SPEED 5
#define HIGH_SPEED 90
#define SLOW_RATE 1200
#define FAST_BEACON_RATE 60
#define TURN_MIN 10
#define TURN_SLOPE 240
#define MIN_TURN_TIME 15
...
void locationUpdate()
{
  ...
  #ifdef REPORT_ALT
  if (ialtitude_feet != 4200) { //check if receiving valid altitude reading.
    char APRS_comment[32]= "/A=";
    sprintf(temp, "%06d", ialtitude_feet);
    strcat(APRS_comment,temp);
    Serial.println("Reporting Altitude");
  } else {
    char APRS_comment[32]= "";
    Serial.print(APRS_comment); //just added to make sure APRS_comment is set to empty char
...
void loop()
{
  ...
  {
    #ifdef BLINK_LEDS
    digitalWrite(GPS_FIX_LED, HIGH); // Toggle the GPS_FIX_LED on
    #endif
    ...
    // Quick fix to ignore wrong data in calculations
    if (currentcourse > 360 || currentcourse < 0)
      currentcourse = 0;
    ...
    Serial.print(F(" "));
    if (faltitude_meters == 1000000.0)
    {
      Serial.print("INVALID");
      Serial.print(F("/"));
      Serial.print("INVALID");
    } else {
      Serial.print(faltitude_meters);
      Serial.print(F("/"));
      Serial.print(ialtitude_feet);
    }
    ...
    #ifdef BLINK_LEDS
    digitalWrite(GPS_FIX_LED, LOW); // Toggle the GPS_FIX_LED off
    #endif
  }
  newData = false;
}
